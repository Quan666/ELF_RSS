...
async def start():
    ...
    try:
        ...
        await bot.send_msg(message_type='private', user_id=str(list(config.superusers)[0]), message='ELF_RSS 订阅器启动成功！\nVERSION: {}'.format(config.version))
        logger.info('ELF_RSS 订阅器启动成功！')
    except Exception as e:
        await bot.send_msg(message_type='private', user_id=str(list(config.superusers)[0]), message='第一次启动，你还没有订阅，记得添加哟！\nVERSION: {}'.format(config.version))
