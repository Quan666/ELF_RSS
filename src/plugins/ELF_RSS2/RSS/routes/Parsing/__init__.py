...
def _sort(_list):
    ...
    for x in _list:
        if x.rex != "(.*)":
            delete.append(x.priority)
    for x in _list.copy():
        if x.priority in delete and x.rex == "(.*)":
            _list.remove(x)
    return _list
    ...
    async def start(self, new_rss: dict, old_data: list):
        ...
        for item in self.state.get("change_data"):
            ...
            for k, v in self.handler.items():
                ...
                tmp_state = {
                    "continue": True  # 是否继续执行后续处理
                }
                # 某一个内容的处理如正文，传入原文与上一次处理结果，此次处理完后覆盖
                for h in v:
                    tmp = await h.func(rss=self.rss, state=self.state, item=item, item_msg=item_msg, tmp=tmp,
                                       tmp_state=tmp_state)
...
async def handle_title(rss: Rss, state: dict, item: dict, item_msg: str, tmp: str, tmp_state: dict) -> str:
    # 处理标题
    title = item["title"]
    res = ""
...
async def handle_summary(rss: Rss, state: dict, item: dict, item_msg: str, tmp: str, tmp_state: dict) -> str:
    tmp += await handle_img(
        html=Pq(get_summary(item)), img_proxy=rss.img_proxy, img_num=rss.max_image_number
    )
    return f'{tmp}\n'
...
async def handle_torrent(rss: Rss, state: dict, item: dict, item_msg: str, tmp: str, tmp_state: dict) -> str:
    if not rss.is_open_upload_group:
        rss.group_id = []
    if rss.down_torrent:
        # 处理种子
        try:
            hash_list = await down_torrent(rss=rss, item=item, proxy=get_proxy(rss.img_proxy))
...
async def handle_date(rss: Rss, state: dict, item: dict, item_msg: str, tmp: str, tmp_state: dict) -> str:
...
async def handle_message(rss: Rss, state: dict, item: dict, item_msg: str, tmp: str, tmp_state: dict) -> str:
    # 发送消息并写入文件
